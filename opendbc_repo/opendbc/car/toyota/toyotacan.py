...
def create_ui_command(packer, steer, chime, left_line, right_line, left_lane_depart, right_lane_depart, enabled, stock_lkas_hud, mads_enabled=False):
  values = {
    "TWO_BEEPS": chime,
    "LDA_ALERT": steer,
    "RIGHT_LINE": 3 if right_lane_depart else 1 if right_line and enabled else 2,
    "LEFT_LINE": 3 if left_lane_depart else 1 if left_line and enabled else 2,
    "BARRIERS": 1 if enabled else 0,
    "LKAS_STATUS": 2 if enabled else 1 if mads_enabled else 0,
